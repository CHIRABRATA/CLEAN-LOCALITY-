    ...
    <Marker
      ...
      eventHandlers={{
        dragend() {
          ...
          if (m) { const { lat, lng } = m.getLatLng(); setPosition([lat, lng]); reverseGeocode(lat, lng); }
        },
      }}
    />
  ) : null;
...
function MapPicker({ setLocation }) {
  ...
  const locateUser = () => {
    ...
    navigator.geolocation.getCurrentPosition(
      ({ coords }) => {
        ...
        setMarkerPos(pos); setFlyCoords(pos); setLocating(false);
  ...
  const handleLocationPicked = useCallback(({ lat, lng, address }) => {
  ...
  return (
    <div style={{ position:"relative", borderRadius:"18px", overflow:"hidden" }}>
      <style>{`
        ...
        .leaflet-control-zoom a{border-radius:8px !important;background:rgba(10,10,20,.85) !important;color:#fff !important;border-color:rgba(255,255,255,.1) !important;margin-bottom:4px !important;}
      ...
      <button onClick={locateUser} disabled={locating} title="Use my location" style={{ position:"absolute",top:10,right:10,zIndex:999,width:36,height:36,borderRadius:"10px",background:"#3B82F6",border:"none",cursor:"pointer",color:"#fff",fontSize:18,display:"flex",alignItems:"center",justifyContent:"center",boxShadow:"0 4px 14px rgba(59,130,246,.5)" }}>
        {locating ? "⟳" : "⌖"}
      </button>
    </div>
  );
...
function NearbyMap({ posts, userLocation }) {
  ...
  return (
    <div style={{ position:"relative", borderRadius:"20px", overflow:"hidden" }}>
          ...
          <Marker key={p.id} position={[p.latitude, p.longitude]} icon={issueIcon}>
            <Popup>
              <div style={{ maxWidth:160 }}>
                <strong>{p.description?.slice(0, 60)}{p.description?.length > 60 ? "…" : ""}</strong>
                <div style={{ color:"#888", marginTop:4, fontSize:11 }}>{p.address?.split(",")[0]}</div>
              </div>
            </Popup>
          </Marker>
        ))}
      ...
      {/* Green = you, Red = issue legend */}
      <div style={{ position:"absolute",bottom:10,right:10,zIndex:999,display:"flex",flexDirection:"column",gap:4,pointerEvents:"none" }}>
        <div style={{ display:"flex",alignItems:"center",gap:5,background:"rgba(0,0,0,.65)",backdropFilter:"blur(6px)",padding:"3px 8px",borderRadius:8,fontSize:10,fontWeight:700,color:"#fff" }}>
          <div style={{ width:8,height:8,borderRadius:"50%",background:"#10B981" }} /> You
        </div>
        <div style={{ display:"flex",alignItems:"center",gap:5,background:"rgba(0,0,0,.65)",backdropFilter:"blur(6px)",padding:"3px 8px",borderRadius:8,fontSize:10,fontWeight:700,color:"#fff" }}>
          <div style={{ width:8,height:8,borderRadius:"50%",background:"#EF4444" }} /> Issues
        </div>
      </div>
    </div>
  );
...
const BADGES  = [
  { min:10, icon:"🔥", label:"Fire Reporter" },
  { min:5,  icon:"⭐", label:"Star Contributor" },
  { min:1,  icon:"🌱", label:"New Voice" },
];
      ...
      <div className="widget-hdr">
        <div style={{ display:"flex",alignItems:"center",gap:8 }}>
          ...
          <div>
            ...
            <div className="widget-sub">Most reports this month</div>
          </div>
        </div>
        {data.length > 3 && (
          <button className="widget-toggle" onClick={onToggle}>
            {expanded ? "Show less ↑" : `+${data.length - 3} more ↓`}
          </button>
        )}
      </div>

      {data.length === 0 ? (
        <div style={{ textAlign:"center",padding:"20px 0",color:"var(--txt3)",fontSize:13,fontWeight:600 }}>
        ...
        <div style={{ display:"flex",flexDirection:"column",gap:8,marginTop:14 }}>
          {visible.map((entry, i) => {
            ...
            return (
              ...
              >
                <span className="lb-rank">{isTop3 ? MEDALS[i] : RANKS[i]}</span>
    ...
    <div className="widget-card">
      <div className="widget-hdr" style={{ cursor:"pointer" }} onClick={onToggle}>
        <div style={{ display:"flex",alignItems:"center",gap:8 }}>
          <span style={{ fontSize:20 }}>💡</span>
          <div>
            <div className="widget-title">Tips for Better Reports</div>
            <div className="widget-sub">Help authorities respond faster</div>
          </div>
        </div>
      ...
      {expanded && (
        <div style={{ marginTop:14,display:"flex",flexDirection:"column",gap:9 }}>
          ...
          <div style={{ marginTop:4,padding:"10px 12px",background:"var(--as)",borderRadius:12,border:"1px solid rgba(59,130,246,.2)" }}>
            <p style={{ fontSize:12,color:"var(--accent)",fontWeight:700,lineHeight:1.5 }}>
              ✨ Good reports get resolved 3× faster and help your community more!
            </p>
          </div>
        </div>
      )}
...
export default function PostPage({ user }) {
    ...
    try {
      ...
      if (user?.id) {
        const { data: voteData } = await supabase
          .from("votes").select("post_id").eq("user_id", user.id);
  ...
  // ── Fetch leaderboard: count posts per user, join citizens for name ──────────
  const fetchLeaderboard = async () => {
    try {
      // Count posts grouped by user_id, join with citizens for name
      const { data, error } = await supabase
        .from("posts")
        .select("user_id, citizens(name)")
        .not("user_id", "is", null);
      ...
      data.forEach(row => {
        ...
        const name = row.citizens?.name || "Anonymous";
  ...
  const uploadImage = async (file) => {
    ...
    if (uploadErr) { console.error("[upload] error:", uploadErr); throw new Error(`Storage: ${uploadErr.message}`); }
  ...
  const handlePost = async () => {
    ...
    try {
      ...
      try {
        ...
        showToast(storageErr.message || "Image upload failed", "error");
        setPublishing(false);
        return;
      }

      console.log("[publish] inserting with user_id:", user.id, "image_url:", imageUrl);
      const { data, error } = await supabase
        .from("posts")
      ...
      if (error) { console.error("[publish] db error:", error); throw new Error(`DB: ${error.message}`); }
      ...
      setStep("feed");
      setSelectedImg(null); setSelectedFile(null); setDescription(""); setLocation({ lat:null, lng:null, address:"" }); setShowMap(false);
      showToast("Report published! ✅");
      fetchLeaderboard(); // refresh leaderboard after new post
    } catch (err) {
  ...
  const statusColors = { pending:"#F59E0B", solved:"#10B981", rejected:"#EF4444" };
  ...
  return (
    <div className={`cp-root ${theme}`}>
      <style>{`
        ...
        .cp-root.dark{--bg:#060608;--bg2:#0f0f14;--surface:rgba(255,255,255,.035);--surface-h:rgba(255,255,255,.06);--border:rgba(255,255,255,.07);--border-s:rgba(255,255,255,.13);--txt:#f0f0f5;--txt2:rgba(240,240,245,.5);--txt3:rgba(240,240,245,.25);--accent:#3B82F6;--ag:rgba(59,130,246,.28);--as:rgba(59,130,246,.1);--danger:#EF4444;--ds:rgba(239,68,68,.13);--nav:rgba(6,6,8,.86);--shadow:0 20px 50px rgba(0,0,0,.55);--mbg:rgba(0,0,0,.88);--mcard:#0f0f14;--tag:rgba(255,255,255,.06);--scroll:rgba(255,255,255,.06);--widget-bg:rgba(255,255,255,.025);--gold:#F59E0B;}
        .cp-root.light{--bg:#f0f2f7;--bg2:#fff;--surface:rgba(255,255,255,.9);--surface-h:#fff;--border:rgba(0,0,0,.07);--border-s:rgba(0,0,0,.13);--txt:#0f0f18;--txt2:rgba(15,15,24,.55);--txt3:rgba(15,15,24,.3);--accent:#2563EB;--ag:rgba(37,99,235,.18);--as:rgba(37,99,235,.08);--danger:#DC2626;--ds:rgba(220,38,38,.1);--nav:rgba(240,242,247,.9);--shadow:0 8px 30px rgba(0,0,0,.08);--mbg:rgba(0,0,0,.55);--mcard:#fff;--tag:rgba(0,0,0,.05);--scroll:rgba(0,0,0,.1);--widget-bg:rgba(255,255,255,.8);--gold:#D97706;}
        ...
        *{box-sizing:border-box;margin:0;padding:0;}
        ...
        .theme-toggle:hover{border-color:var(--accent);box-shadow:0 0 0 3px var(--as);}
        ...
        .cp-fab:active{transform:translateX(-50%) scale(.95);}
        ...
        .publish-btn:disabled{opacity:.45;cursor:not-allowed;transform:none !important;box-shadow:none;}
      ...
      <main className="cp-feed">
        <div className="cp-feed-hdr">
          ...
          <p className="cp-feed-sub"><span>{feedLoading ? "…" : posts.length} active issues</span> near you</p>
        </div>
              ...
              <div>
                ...
                <div className="widget-sub">
                  {posts.filter(p => p.latitude && p.longitude).length > 0
                    ? `${posts.filter(p => p.latitude && p.longitude).length} mapped issues`
                    : "Be the first to pin one!"}
                </div>
              </div>
            </div>
            <div style={{ fontSize:10,fontWeight:700,color:"var(--txt3)",background:"var(--tag)",padding:"4px 10px",borderRadius:20,border:"1px solid var(--border-s)" }}>
              🟢 You &nbsp;🔴 Issues
            </div>
          </div>
          <NearbyMap posts={posts} userLocation={userLocation} />
        </div>
        ...
        {!feedLoading && posts.length === 0 && (
          <div className="empty-state">
            ...
            <p>No reports yet. Tap 📸 to be the first!</p>
          </div>
        )}
